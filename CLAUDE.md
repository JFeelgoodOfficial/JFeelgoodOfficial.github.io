...
- **Vercel keeps every deployment it builds, in full, until someone deletes it.** The "deployment storage" number on the usage page is that history, not the site: in September 2026 it read 3.5 GB while the site was 15 MB, because 39 retained builds still carried the 77 MB pre-shrink tree. Delete old deployments from the project's Deployments page (or `vercel remove`), keep only the live build and one rollback target, and set the project's Deployment Retention policy short so it never regrows. Each merge to `main` adds one build of the whole tree.
- `.vercelignore` leaves the files no visitor requests (`CLAUDE.md`, `README`, `.nojekyll`, `.claude/`, `.github/`) out of every build, and `ignoreCommand` in `vercel.json` skips the build entirely when a push touches nothing else. The two lists must stay identical.
- A painting ships at one size per role and never twice at the same role. If a work already has a 1200px `opt/` file, its archive click-through points there (`ARCHIVE_FULL` in `content.js`) instead of getting a second 1400px copy under `archives/full/`.
...
- **The court is a room too.** The sunset court is roofless, so for a long time the sky was the only thing lighting it: three lamps against the north wall, nothing at all over its floor, and five of the eight pool slots sitting idle whenever a visitor stood in the middle of the building. It now carries its own fittings — a cove and four downlights under the south beam, a cove along the canopy soffit, and a line along each pool curb. Two rules came out of placing them: a fixture sharing a column's x sits *inside* the column and blows it into a lit pillar (the polished paving then mirrors the streak), and a single lamp mid-curb throws its hotspot straight down the court's centreline. Keep them between the columns and off the centreline.
...
- **Lights are a shader budget, not a scene graph.** three.js unrolls `#pragma unroll_loop_start` on the JS side, so every light in the scene pastes another full GGX evaluation into every lit fragment shader; past roughly a dozen, mobile drivers stop linking the program and the whole building draws nothing. So `building.js` emits fixtures as *data* (`fixture(x, y, z, intensity, color, range)`) and `lights.js` keeps a fixed pool of real `PointLight`s aimed at the ones nearest the visitor. Add a light by adding a fixture; never add a `THREE.PointLight` to the scene. `C.LIGHT_POOL` is the cap per tier and `C.WING_LIGHT_*` the corridor spacing — the pool must be big enough to hold every fixture whose falloff still matters where the visitor can stand, or lights visibly swap in and out. `WING_LIGHT_RANGE` is a hard cutoff rather than a fade: past it a fixture contributes exactly nothing, so a reach short of the room's length leaves the corridor ahead lit only by the indoor fill and the lamps never appear to come on. A wing is 84 m long; the reach is 48 m and the desktop pool holds ten, because at eight the ninth and tenth fixtures were still carrying real light down the room. Range is a uniform, so lengthening it costs no shader budget; the pool size does. Changing the pool size recompiles every lit program, so it only ever changes on the failure ladder in `degradeLighting()`.
